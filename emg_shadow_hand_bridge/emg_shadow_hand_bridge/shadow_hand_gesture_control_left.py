...
ADDUCTION_POSE  = np.zeros(N, np.float32)
ADDUCTION_POSE[2]  = d2r(-10.0)    # FFJ3
ADDUCTION_POSE[6]  = d2r(-4.0)     # MFJ3
ADDUCTION_POSE[10] = d2r(4.0)    # RFJ3
ADDUCTION_POSE[15] = d2r(10.0)   # LFJ3
ADDUCTION_POSE[19] = d2r(26.0)   # THJ4
ADDUCTION_POSE[21] = d2r(-12.0)    
ADDUCTION_POSE[22] = d2r(-33.0)    
ADDUCTION_POSE[23] = d2r(-48.0)    
ADDUCTION_POSE = clamp(ADDUCTION_POSE)
...
SUPINATION_POSE = DEFAULT_POSE.copy() 
PRONATION_POSE  = DEFAULT_POSE.copy() 
...
class HandNode(Node):
    ...
    EMG classifier output:
      - /emg/gesture_idx (Int32): 0..9  (stable after decision layer)
      - (optional) /emg/gesture (String): "Label (0.95)"
      - (optional) /emg/gesture_confidence (Float32)
    """
    def __init__(self):
        super().__init__("shadow_hand_control_left")

        # ---- parameters ----
        self.declare_parameter("topic", "joint_command_left")
        self.declare_parameter("rate_hz", 100.0)          
        self.declare_parameter("vmax_pose", 1.0)        
        self.declare_parameter("gesture_idx_topic", "/emg/gesture_idx_left")
        self.declare_parameter("gesture_topic", "/emg/gesture_left") 
        self.declare_parameter("use_string_gesture", False)      
        self.declare_parameter("debounce_sec", 0.05)       

        self.topic  = str(self.get_parameter("topic").value)
        rate_hz     = float(self.get_parameter("rate_hz").value)
        self.dt     = 1.0 / max(1e-6, rate_hz)            # ✅ FIX
        ...
        # ---- idx -> pose mapping ----
        # 0..9: Rest, Extension, Flexion, Ulnar, Radial, Fist, Abduction, Adduction, Supination, Pronation
        self.IDX2POSE = {
            0: DEFAULT_POSE,      # Rest
            1: EXT_POSE,          # Extension
            2: FLEX_POSE,         # Flexion
            3: ULNAR_POSE,        # Ulnar Deviation
            4: RADIAL_POSE,       # Radial Deviation
            5: FIST_POSE,         # Fist
            6: ABDUCTION_POSE,    # Abduction
            7: ADDUCTION_POSE,    # Adduction 
            8: SUPINATION_POSE,   # Supination 
            9: PRONATION_POSE,    # Pronation 
        }


        self.get_logger().info(
            "Controls:\n"
            " [e]= START EMG control | [r]= STOP (freeze)\n"
            " q=Quit\n"
            f" Subscribing gesture idx from: {self.gidx_topic}"
            + (f" | string: {self.gstr_topic}" if self.use_string else "")
        )
    ...
    def _handle_keys(self):
        while not self.keyq.empty():
            ...
            if   k == 'e':
                ...
                self._start_pose(EXT_POSE, "→ Extension")
            elif k == '2':
                self._start_pose(FLEX_POSE, "→ Flexion")
            elif k == '3':
                self._start_pose(ULNAR_POSE, "→ Ulnar deviation")
            elif k == '4':
                self._start_pose(RADIAL_POSE, "→ Radial deviation")
            elif k == '5':
                self._start_pose(FIST_POSE, "→ Fist")
            elif k == '6':
                self._start_pose(ABDUCTION_POSE, "→ Wrist abduction")
            elif k == '7':
                self._start_pose(ADDUCTION_POSE, "→ Wrist adduction")
            elif k == '8':
                self._start_pose(SUPINATION_POSE, "→ Supination")
            elif k == '9':
                self._start_pose(PRONATION_POSE, "→ Pronation")
            elif k == 'q':
                self._shutdown()
    ...
    def _on_gesture_idx(self, msg: Int32):
        ...
        pose = self.IDX2POSE.get(idx, None)
        if pose is None:
            self.get_logger().warn(f"[EMG] unknown idx={idx}")
            return

        self._last_idx = idx
        self._last_cmd_time = now
        self._start_pose(pose, f"[EMG] idx={idx} → pose")
