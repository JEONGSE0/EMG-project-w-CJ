...
def write_json_inplace(path: str, data: Dict[str, float]) -> None:
    """
    Overwrite an *existing* JSON file in-place (no tmp/rename).
    This works even when the directory is not writable, as long as the file itself is writable.
    """
    if not os.path.isfile(path):
        raise FileNotFoundError(f"cmd.json does not exist: {path}")

    # Compact JSON = shorter write window (less chance reader catches mid-write)
...
class JointStateToJson(Node):
    def __init__(self):
        super().__init__("so101_real_control")

        # ---- params ----
        self.declare_parameter("in_topic", "/so101/joint_states")
        self.declare_parameter("out_path", "/home/gkim451/ros2_ws/src/so101/gkim/demo/cmd.json")
        self.declare_parameter("write_hz", 30.0)
        self.declare_parameter("assume_deg", True)   # True: input is deg; False: input is rad -> convert to deg
